...
var (
	// UsedTIMESTAMPFormat 当前时间展示的格式
	UsedTIMESTAMPFormat = TIMESTAMP
	// UsedTimestampLength 当前使用的时间戳长度
	UsedTimestampLength = Length13
	// UsedTextFormat 文本格式
	UsedTextFormat = "2006-01-02 15:04:05"
)
...
func (t *Time) parseText(data string) error {
	now, err := time.ParseInLocation(`"`+UsedTextFormat+`"`, data, time.Local)
	if err != nil {
		return err
	}
	*t = Time(now)
...
func (t Time) formatText() []byte {
	b := make([]byte, 0, len(UsedTextFormat)+2)
	b = append(b, '"')
	b = time.Time(t).AppendFormat(b, UsedTextFormat)
	b = append(b, '"')
	return b
}
