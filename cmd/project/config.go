...
func (p *Project) LoadPcenterConfig() error {
	...
	var pcenterAddr string
	err := survey.AskOne(
		&survey.Input{
			Message: "Pcenter GRPC服务地址:",
	...
	if strings.Contains(pcenterAddr, ":") {
		hp := strings.Split(pcenterAddr, ":")
		p.Pcenter.Host = hp[0]
		p.Pcenter.Port = hp[1]
	}
	...
	err = survey.AskOne(
		&survey.Password{
			Message: "Pcenter Client Secret:",
		},
		&p.Pcenter.ClientSecret,
		survey.WithValidator(survey.Required),
	)
	if err != nil {
		return err
